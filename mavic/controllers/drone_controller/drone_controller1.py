...
# === WAYPOINT AYARI ===
# ============================================
X_START = 28.0    # X başlangıç (batı)
X_END = 50.0     # X bitiş (doğu)
Y_START = 24.0    # Y başlangıç (kuzey)
Y_END = 32     # Y bitiş (güney)
...
while robot.step(timestep) != -1:
    ...
    if abs(dx) > 0.3:
        pitch_disturbance = -dx * 0.2 # X yönünde (çok düşük)
    if abs(dy_w) > 0.3:
    ...
    if camera and display:
        ...
        if image and frame_count % 5 == 0:
            ...
            for py_cam in range(0, h, 2):
                for px_cam in range(0, w, 2):
